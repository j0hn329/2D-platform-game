...
public class LevelOne extends Level {
	...
	private Animation ghost_right;
	private Animation ghost_left;
	...
	public void init() {
		...
		tmapBackdrop = new TileMap();
		tmapBackdrop.loadMap("maps", "backdropMap.txt");
	...
	public void addEnemies() {
		...
		ghost_right = new Animation();
		ghost_right.loadAnimationFromSheet("images/Sprite/ghost.png", 3, 1, 60);
		ghost_left = new Animation();
		ghost_left.loadAnimationFromSheet("images/Sprite/ghost_left.png", 3, 1, 60);
		ghost_left.setAnimationSpeed(0.3f);
	
		for(int i = 0; i < points.length; i++) {
			
			ghost = new Ghost(ghost_right);
			ghost.setAnimationSpeed(0.3f);
			ghost.setX(points[i].x);
			ghost.setY(points[i].y);
			ghost.setVelocityX(0.0f);
			ghost.setVelocityY(0.0f);
			ghost.show();
			enemies.add(ghost);	
		}
	...
	public void checkSprites(long elapsed) {
		...
		if(spriteBounds(player).intersects(spriteBounds(flag))) {
			...
			portal.start();
			levelManager.setLevel(LevelManager.levelTwo);
		}
	}
	...
	public void update(long elapsed) {
		...
		if(paused) {
			...
			for (int i = 0; i < enemies.size(); i++) {
				...
				if(e.getVelocityX() == 0 && e.getAnimation().equals(ghost_right)) {
					e.setVelocityX(-0.05f);
					e.setAnimation(ghost_left);
				}
				if(e.getVelocityX() == 0 && e.getAnimation().equals(ghost_left)) {
					e.setVelocityX(0.05f);
					e.setAnimation(ghost_right);
				}
				if(e.isDead()) {
		    ...
		    for (int i = 0; i < fireAttack.size(); i++) {
		    		FireAttack fa = fireAttack.get(i);
		  			fa.update(elapsed);
	//	  			*Supposed to give fireAttack tile map collision but is commented out as it doesn't work properly  
	...
	public void draw(Graphics2D g) {
        ...
        xOFF = Math.max(game.WIDTH - tmap.getPixelWidth(), xOFF);

        yOFF = game.HEIGHT/2 - (int)(player.getY());
        yOFF = Math.min(yOFF, 0);
        yOFF = Math.max(game.HEIGHT - tmap.getPixelHeight(), yOFF);
        ...
        tmap.draw(g, xOFF, yOFF);
        tmapBackdrop.draw(g, xOFF, yOFF);
...
//			g.draw(spriteBounds(c));
	...
	public void collision(Sprite s, long elapsed) {
		  ...
		  if (s.getX() < 0) {
	        ...
	        else if (s.getX() + s.getWidth() >= tmap.getPixelWidth())
	        {
	            s.setVelocityX(-Math.abs(s.getVelocityX()));
	        }
	        if (s.getY() < 0) {
		    ...
		    yc = tmap.getTileYC(currentColumn,currentRow);
			xc = tmap.getTileXC(currentColumn,currentRow);
			...
			char topLeft = tmap.getTileChar(leftTile,topTile);
			String topLeftTile = String.valueOf(topLeft);
			
			char topRight = tmap.getTileChar(rightTile,topTile);
			String topRightTile = String.valueOf(topRight);
			
			char bottomRight = tmap.getTileChar(rightTile,bottomTile);
			String bottomRightTile = String.valueOf(bottomRight);
			
			char bottomLeft = tmap.getTileChar(leftTile,bottomTile);
